    ...
    public class func handleOpenURL(_ url: URL) -> Bool {
        ...
        if urlScheme.hasPrefix("wx") {
            ...
            if let data = UIPasteboard.general.data(forPasteboardType: "content") {
                if let dict = ((try? PropertyListSerialization.propertyList(from: data, options: PropertyListSerialization.MutabilityOptions(), format: nil) as? [String: Any]) as [String : Any]??) {
        ...
        if let appID = account?.appID, urlScheme == "ap" + appID || urlScheme == "apoauth" + appID {
            ...
            if urlString.contains("//safepay/?") {
                ...
                guard
                    ...
                    let dict = ((try? PropertyListSerialization.propertyList(from: data, options: PropertyListSerialization.MutabilityOptions(), format: nil) as? [String: Any]) as [String : Any]??),
